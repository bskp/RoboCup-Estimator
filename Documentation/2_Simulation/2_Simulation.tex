...
This chapter focuses on the implementation of the plot functions and the computation functions, i.e. the perturbed dynamics of the ball and the noised measurement. The filter task of the simulation will be discussed in chapter 4.

% -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  - %
\section{Plots}
Plotting a step in the simulation can be divided essentially in two parts. One part contain the static part e.g. the field an the other part all dynamic objects, as the robots and the ball.

\subsection*{Plot of the field - \texttt{plot\_env()} }
All graphical features concerning the playing field are implemented by the function {\fontfamily{pcr}\selectfont plot\_env()}. It is subdivided in two functions, which draw the field and, as a neat add on, the scorecounter seperately. The size of the field and of the goals we took from the RoboCup Nao Rule Book 2011 and are set in the global Struct \texttt{Field}.
\begin{lstlisting}
global Field;
    Field.width = 6; %[m]
    Field.height = 4; %[m]
...
To draw the field we mostly use built-in MATLAB commands such as {\fontfamily{pcr}\selectfont rectangle()} or {\fontfamily{pcr}\selectfont line()}. Only for drawing circles we made the function
...
Figure \ref{Playing_field} shows the playing field after the execution of {\fontfamily{pcr}\selectfont plot\_env()}
...
For the plot of the robots and ball we made the function
...
Figure \ref{Playing_field_robots} shows the playing filed after initialization of \texttt{plot\_env()} and the robots and the ball with the shape parameters \texttt{'0-t'}. Where \texttt{Robot} and \texttt{Ball} are the standard initialization structs as described in Section \ref{Robotsection} and \ref{Ballsection}.

\begin{figure}[h]
	\centering
    	\includegraphics[width=12cm]{./2_Simulation/playing_field_robots}
  	\caption{Playing field after initialization and \texttt{plot\_objects(Robot,Ball,'0-t')}.}
...
\subparagraph{Random Collision Roboter} To satisfy the boundary conditions we first used robots which collide into the sidelines and into each other. When they hit each other they swap direction 
%. In {\fontfamily{pcr}\selectfont dummy\_step(Robot).m} we used a very simple model, assuming that if two robots collide, their directions swap such that they do not run into each other
...
\]
Those dynamics are implemented in \texttt{robot\_rand\_step(Robot)} with a random inputs $d\omega_{i_k}$ and constant velocities $v_{i_k} = v$. But this solution was not optimal for two reasons: First we had the problem that there was a bug which made it possible that two robots became wedged together and their further behaviour was messed up after they had contact. The second problem was, that our Kalman filter didn't work properly since the swapping of directions is a rapid change in states, which cannot be handled by our Kalman filter. 

\subparagraph{Random Potential Roboter} 
The function {\fontfamily{pcr}\selectfont robot\_randpot\_step(Robot,Ball)} eliminates both issues because it doesn't use collision detection but collision avoidance. If the distance of two robots falls below a given radius, we assign a potential to both robots. The effect is now similar to this of a positive charge in an electrostatic field: The closer two robots are, the bigger is their mutual repulsion. The force in a electric field is given by
...
Or the same described in vector notation:
	...
	\]
As you can in the code below, we use the same \(r^{-2}\)-relation for the repulsing force as it is known from the analysis of electrostatic fields
\begin{lstlisting}
        for j=1:8
...
\begin{center}
	...
	\end{tabular}
\end{center}
 These parameters are set by executing {\fontfamily{pcr}\selectfont ball\_init()}. This function also defines the ball's radius, its initial velocity and the friction towards the ground in the global struct \texttt{BallParam}
 
\subsection*{Dynamics of the ball - Mathematical model}
%The function {\fontfamily{pcr}\selectfont ball\_step(Ball,Robot).m} does, like the equivalent function for the robots, the computations of the ball's next position and direction on the field. These parameters however do not only depend on the ball's dynamics but also whether it collides with one of the robots. The following MATLAB code shows the algorithm for this collision detection

\lstinputlisting[firstline=40, lastline=51]{../Simulation/Merge/ball_step.m}
The dynamics of the ball are quite analog to those of the robots. Only the velocity is no longer an input but a state.
The the non-linear motion equation for the ball are
\begin{eqnarray*}
...
With the boundary conditions, that the boll has to stay on the playing field and can't be under a robot
	...
	\]
To satisfy these conditions, we used again the law of reflection when the ball hits one of the sideline. and when collide with
