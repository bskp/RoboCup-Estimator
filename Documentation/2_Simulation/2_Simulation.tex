...
\begin{description}
	...
	\item[\texttt{style}] is a string defining the robotershape, which contain an arbitrary number and order of following characters\\
		...
		and one of the following character to specify the color\\
		\texttt{'t'} Team specific colors (blue and magenta) \\
\end{description}
Figure text color{red}{blabla} shows the playing filed after the execution of \texttt{plot\_env()} and \texttt{plot\_objects(Robot,Ball,'0-t')}.
...
\lstinputlisting[firstline=35, lastline=46]{../Simulation/Merge/dummyPro_step.m}
